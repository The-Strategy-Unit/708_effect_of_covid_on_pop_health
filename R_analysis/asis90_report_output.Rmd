...
>In the calendar year 2020 there were `r print_num(yx2020CovidDths)` deaths involving Covid-19 &mdash; `r print_num(yx2020CovidDthsPc)` of the total `r print_num(yx2020Dths)` deaths that occurred in England & Wales.
...
```{r covid-dths-data, fig.cap="Early in the pandemic, surveillance reporting did not include <br> deaths outside hospital", fig.topcaption=TRUE}
onsDths <- onsDths %>% 
  rownames_to_column() %>%
...
```{r deaths-last-5-years, fig.cap="The numbers of all-cause deaths in Spring 2020 and Winter 2020-21 <br> were dramatically increased compared with recent years", fig.topcaption=TRUE}
plotDat <- wkDthsTot %>%
  filter(isoYr > 2014) %>% 
  mutate(
...
```{r model-dths, fig.cap="A statistical model can generate a prediction of 'normal' deaths <br> against which to measure the impact of the coronavirus pandemic", fig.topcaption=TRUE}
plotDat <- wkDthsTot %>%
  filter(isoYr >= 2019) %>%
  select(isoYrWk, dths) %>%
...
>We estimate there have been `r print_num(totCovidDths, d = 0)` excess deaths in England & Wales from the start of the pandemic (by week 17 2021).

```{r methods-xsdths-tab, fig.cap="Different methods for estimating excess deaths arrive at broadly similar results"}
tabDat <- xsDthsSumTb %>% 
...
```{r qxSmr, fig.cap="The first and second waves of Covid-19 both caused large increases in standardised mortality rates", fig.topcaption=TRUE}
plotDat <- isoWksSeries %>% 
