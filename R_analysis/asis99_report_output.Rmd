...
# knitr options
opts_chunk$set(echo = TRUE, warning = FALSE, eval.after = "fig.cap")
...
>In the calendar year 2020 there were `r print_num(yx2020CovidDths)` deaths involving Covid-19 &mdash; `r print_num(yx2020CovidDthsPc)` of the total `r print_num(yx2020Dths)` deaths that occured in England & Wales.
...
subtitle_lab <- "Covid-19 weekly death count, <span style='color:#3974b3;'>death certification</span><span style='color:#686f73;'> v. </span><span style='color:#ca3923;'>surveillance</span>"
...
```{r covid-effects, fig.cap="Excess mortality is influenced by many factors", fig.topcaption=TRUE, out.width="100%", out.extra='style="margin-bottom:20px;"'}
...
subtitle_lab <- "Weekly all-cause death count, <span style='color:#3974b3;'>mean 2015&ndash;2019</span><span style='color:#686f73;'>, </span><span style='color:#ca3923;'>2020</span><span style='color:#686f73;'> and </span><span style='color:#792215;'>2021</span>"
...
subtitle_lab <- "Weekly all-cause death count, <span style='color:#3974b3;'>2019</span><span style='color:#686f73;'>, </span><span style='color:#ca3923;'>2020</span><span style='color:#686f73;'> and </span><span style='color:#792215;'>2021</span>"
...
Table \@ref(tab:xsdths-tab) compares the estimates of excess deaths from our three alternative approaches. There is no definitive answer to which approach provides the most accurate estimate of the pandemic's impact. However, we share the view of the Institute and Faculty of Actuaries (IFoA) that the similarity between mortality in early-2019 and early-2020 presents a strong case for favouring the use of 2019 deaths as the benchmark for expected deaths in 2020 and 2021.[@continuous_mortality_investigation_england_2020]
...
tabDat <- xsDthsSumTb %>% 
  ...
  rename(`Method` = wave
         , `Deaths from 2019` = dths19
         , `Average deaths 2015-19` = mn5
         , `Modelled deaths` = glmFmFit) %>%
...
>We estimate the coronavirus pandemic led to `r print_num(totCovidDths, d = 0)` extra deaths (by week 17 2021).
...
```{r non-covid-dths, fig.cap="The only time non-Covid-19 deaths were consistently above normal levels was in Spring 2020", fig.topcaption=TRUE}
...
```{r xsDths-tab, tab.cap="In the first wave there were more excess deaths than Covid deaths; this relationship reversed for the second wave"}
tabDat <- xsDthsSumTb %>%
  filter(wave != "Total") %>% 
  select(wave, dths19) %>%
...
lab <- "Cumulative excess deaths: <span style='color:#3974b3;'>men </span><span style='color:#686f73;'>& </span><span style='color:#ca3923;'>women</span>"
...
delete_no_display <- function(x) {
  if_else(str_detect(x, "_no_display"), "", str_replace(x, "^.*[0-9]{2}([0-9]{2})(-W)([0-9]{2}).*", "\\1-W\\3"))
}
...
```{r qxSmr, fig.cap="Coronavirus caused large increases in standardised mortality rates", fig.topcaption=TRUE}
plotDat <- isoWksSeries %>% 
...
>Using standard life tables for England & Wales, we estimate `r print_num(yllTot / 1e6, d = 2)` million years of life have been lost to Covid (week `r recentWk` 2021).
...
```{r icnarc-tab, tab.cap = "On admission patients critically ill with Covid-19 were healthier than patients with viral pneumonia"}
